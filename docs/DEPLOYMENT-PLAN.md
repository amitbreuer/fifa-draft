...
7. **Every authenticated API call returned 401 in Telegram** — modern Telegram Mini App `initData` includes a `signature` field (Ed25519, for third-party validation). The HMAC data-check-string must exclude **both** `hash` **and** `signature`; the middleware only removed `hash`, so every real client failed validation. Symptom: `OPTIONS 204` then `GET/POST … 401` in Cloud Run logs, "Failed to create draft" in the UI. Fix: `params.delete('signature')` alongside `params.delete('hash')` in `validateInitData` (`projects/server/src/middleware/auth.ts`). Server-side change → needs a Cloud Run redeploy.
